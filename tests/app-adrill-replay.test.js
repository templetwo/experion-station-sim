...
test('A1 TRAINING replay reproduces retained events and score (gate 3)', () => {
  ...
  assert.equal(beforeEvents.length, 4, 'test setup: expected four accepted TRAINING actions, got ' + beforeEvents.length);
...
test('A-drill Debrief submit earns the debrief category from the UI path', () => {
  ...
  assert.equal(v.arch.debrief.ask, true, 'YES/NO must be offered while an A-drill is active');
  v.arch.debrief.yes();
  const after = DrillArch.scoreDrill('A1', c.P.aDrill.events.slice());
