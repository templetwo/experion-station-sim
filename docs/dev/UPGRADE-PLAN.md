...
4. Keep every existing feature working: three units, drills D1–D12, command zone, F-keys, faceplates, Point Detail, trends, security levels, Ops Assistant. The existing trip thresholds stay unless a cited source justifies a change. There are **six**: 98 % TK-101, 185 °C R-201, 950 kPa PSV V-401, 110 °C R-202, 480 °C R-310 bed, 1100 kPa PSV V-502. All six are raised through `raiseTrip()` in `src/models.js` — **grep that symbol for the live list rather than trusting this one.** (Corrected 2026-09-13: this rule listed five until Unit 04's V-502 relief shipped in 3.1.0 on 2026-09-03 and was never added. The count is now derived from the code, so it cannot go stale again.)
