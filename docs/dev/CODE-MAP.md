...
- **Indication**: `almSty(a)` uses `engine.indication(state, prio)`: UNACK blinks, RTNUN inverse-blinks, ACKED steady, SHLVD/DSUPR/OOSRV grey. Horn only for UNACK (`hornTop`). Counters `cU/cH/cL` = `engine.counts()` (UNACK+ACKED, RTNUN excluded from active counts by the engine).
- **State**: `shelfView` now also `SUPPRESSED` | `OOS`. Shelve dialog state `dlg:{type:'shelve', dur, reasonSel}` with `dlgReason` as free-text detail.
